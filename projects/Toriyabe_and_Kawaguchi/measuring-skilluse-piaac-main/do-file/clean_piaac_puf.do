...
// 		svk esp swe gbr usa chl grc isr ltu nzl sgp svn tur {
//		
// 		//cyp  
// 	if (`cnt'==9) {
// 		local `cnt++'
...
// このStataのコードは、1から49までの数値をnumに代入し、clX変数が7と等しい場合にその値を0に置き換えます。
//
// 具体的には、replaceコマンドを使用して、以下の処理が行われます：
//
// clX変数が7と等しい場合、その値が0に置き換えられます。
// num変数が1から49までの数値であるため、この処理はそれぞれのnumについて行われます。
// 結果的に、clX変数が7である値が0に置き換えられます。
...
save "${path_data}/until_676.dta", replace


foreach i of numlist 1(1)8 10(1)10 12(1)16 18(1)22 24(1)30 {
	display `i'
...
save "${path_data}/until_780.dta", replace
...
save "${path_data}/until_898.dta", replace
